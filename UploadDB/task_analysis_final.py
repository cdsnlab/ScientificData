...
def task_analysis(data_path, actuator_list, environment_list, single_tasks, projector_tasks, ignore_brightness=False):
    ...
    for task_name, related_fnames in task_to_fname.items():
        ...
        for fname in related_fnames:
            ...
            publishers = list(set(sensor_df['publisher']))
            environment_agents = list(filter(lambda x: x in environment_list, publishers))
            actuator_agents = list(filter(lambda x: x in actuator_list, publishers))

            task_to_sensor[task_name].append([fname, environment_agents, actuator_agents])

            brightness_medians = [sensor_df[sensor_df.name =='sensor0_Brightness']['value'].median(), \
                sensor_df[sensor_df.name =='sensor1_Brightness']['value'].median()]

            if 'AmbientSensorAgent' in environment_agents:
                if brightness_medians[0] == np.NaN or (brightness_medians[0] < 3 and brightness_medians[1] < 3):
                    task_to_sensor[task_name][-1][1].remove('AmbientSensorAgent')
                # task_to_sensor[task_name][-1].append(brightness_medians)
        
        if task_name in projector_tasks:
            if ignore_brightness:
                task_to_sensor[task_name] = sorted(task_to_sensor[task_name], key=lambda x: (x[2].count('ProjectorAgent'), len(x[1]), x[1].count('SoundSensorAgent'), len(x[2])), reverse=True)
            else: 
                task_to_sensor[task_name] = sorted(task_to_sensor[task_name], key=lambda x: (-(x[3].count(0.0)+x[3].count(np.NaN)), x[2].count('ProjectorAgent'), len(x[1]), x[1].count('SoundSensorAgent'), len(x[2])), reverse=True)
        else:
            if ignore_brightness:
                task_to_sensor[task_name] = sorted(task_to_sensor[task_name], key=lambda x: (len(x[1]), x[1].count('SoundSensorAgent'), len(x[2])), reverse=True)
            else:
                task_to_sensor[task_name] = sorted(task_to_sensor[task_name], key=lambda x: (-(x[3].count(0.0)+x[3].count(np.NaN)), len(x[1]), x[1].count('SoundSensorAgent'), len(x[2])), reverse=True)
